class Account < ApplicationRecord
  ...
  # :confirmable, :lockable, :timeoutable, :trackable and :omniauthable
    ...
    def self.find_or_create_default_account(email)
      Account.find_by_email(email) ? Account.find_by_email(email) : Account.create(email: email, password: "Test@123")
    end
    
    private
    def valid_weightage_percentage?
      byebug
      (100 - (self.nominees.sum(&:weightage)) >= params[:nominee][:weightage].to_f)
